...
var readability = {
     ...
     * 
     * Workflow:
     *  1. Prep the targdoc by removing script tags, css, etc.
...
//        window.onload = window.onunload = function() {};
        ...
        readability.removeScripts(targdoc);

        if(targdoc.body && !readability.bodyCache) {
            readability.bodyCache = targdoc.body.innerHTML;

        }
        /* Make sure this targdoc is added to the list of parsed pages first, so we don't double up on the first page */
        readability.parsedPages[window.location.href.replace(/\/$/, '')] = true;

        /* Pull out any possible next page link first */
        var nextPageLink = readability.findNextPageLink(targdoc.body);
        
        readability.prepDocument();

        /* Build readability's DOM tree */
        var overlay        = targdoc.createElement("DIV");
        var innerDiv       = targdoc.createElement("DIV");
        ...
        if(!articleContent) {
            articleContent    = targdoc.createElement("DIV");
        ...
        targdoc.body.className = readStyle;
        targdoc.dir            = readability.getSuggestedDirection(articleTitle.innerHTML);
        ...
        /* Glue the structure of our targdoc together. */
        ...
        targdoc.body.innerHTML = "";
        targdoc.body.insertBefore(overlay, targdoc.body.firstChild);
        targdoc.body.removeAttribute('style');

        if(readability.frameHack)
        {
            var readOverlay = targdoc.getElementById('readOverlay');
        ...
        **/
        if((window.location.protocol + "//" + window.location.host + "/") === window.location.href)
        {
            ...
            var rootWarning = targdoc.createElement('p');
                ...
                rootWarning.innerHTML = "<em>Readability</em> was intended for use on individual articles and not home pages. " +
                    "If you'd like to try rendering this page anyway, <a onClick='javascript:targdoc.getElementById(\"readability-warning\").style.display=\"none\";targdoc.getElementById(\"readability-content\").style.display=\"block\";'>click here</a> to continue.";
        ...
        targdoc.onkeydown = function(e) {
            ...
            if (code === 32) {
                ...
                var windowHeight = window.innerHeight ? window.innerHeight : (targdoc.targdocElement.clientHeight ? targdoc.targdocElement.clientHeight : targdoc.body.clientHeight);
    ...
    **/
    ...
    getArticleTools: function () {
        var articleTools = targdoc.createElement("DIV");
    ...
    getArticleTitle: function () {
        ...
        try {
            curTitle = origTitle = targdoc.title;
            
            if(typeof curTitle !== "string") { /* If they had an element with id "title" in their HTML */
                curTitle = origTitle = readability.getInnerText(targdoc.getElementsByTagName('title')[0]);             
            }
        }
        catch(e) {}
        ...
        {
            var hOnes = targdoc.getElementsByTagName('h1');
            if(hOnes.length === 1)
            {
        ...
        var articleTitle = targdoc.createElement("H1");
    ...
    getArticleFooter: function () {
        var articleFooter = targdoc.createElement("DIV");
        ...
        "<div id='rdb-footer-print'>Excerpted from <cite>" + targdoc.title + "</cite><br />" + window.location.href + "</div>",
    ...
    /**
     * Prepare the HTML targdoc for readability to scrape it.
    ...
    prepDocument: function () {
         ...
         * so we create a new body node and append it to the targdoc.
         */
        if(targdoc.body === null)
        {
            var body = targdoc.createElement("body");
            try {
                targdoc.body = body;       
            }
            catch(e) {
                targdoc.targdocElement.appendChild(body);
                dbg(e);
            }
        }

        targdoc.body.id = "readabilityBody";

        var frames = targdoc.getElementsByTagName('frame');
        if(frames.length > 0)
        {
            var bestFrame = null;
            ...
            for(var frameIndex = 0; frameIndex < frames.length; frameIndex+=1)
            {
                ...
                try {
                    var frameBody = frames[frameIndex].contentWindow.targdoc.body;
                    canAccessFrame = true;
                }
                catch(eFrames) {
                    dbg(eFrames);
            ...
            if(bestFrame)
            {
                var newBody = targdoc.createElement('body');
                newBody.innerHTML = bestFrame.contentWindow.targdoc.body.innerHTML;
                newBody.style.overflow = 'scroll';
                targdoc.body = newBody;
                
                var frameset = targdoc.getElementsByTagName('frameset')[0];
                if(frameset) {
        ...
        for (var k=0;k < targdoc.styleSheets.length; k+=1) {
            if (targdoc.styleSheets[k].href !== null && targdoc.styleSheets[k].href.lastIndexOf("readability") === -1) {
                targdoc.styleSheets[k].disabled = true;
            }
        }

        /* Remove all style tags in head (not doing this on IE) - TODO: Why not? */
        var styleTags = targdoc.getElementsByTagName("style");
        ...
        targdoc.body.innerHTML = targdoc.body.innerHTML.replace(readability.regexps.replaceBrs, '</p><p>').replace(readability.regexps.replaceFonts, '<$1span>');
    },

    /**
     * For easier reading, convert this targdoc to have footnotes at the bottom rather than inline links.
     * @see http://www.roughtype.com/archives/2010/05/experiments_in.php
     *
     * @return void
    **/
    addFootnotes: function(articleContent) {
        var footnotesWrapper = targdoc.getElementById('readability-footnotes'),
            articleFootnotes = targdoc.getElementById('readability-footnotes-list');
        
        if(!footnotesWrapper) {
            footnotesWrapper               = targdoc.createElement("DIV");
            ...
            articleFootnotes    = targdoc.createElement('ol');
            articleFootnotes.id = 'readability-footnotes-list';
            
            footnotesWrapper.appendChild(articleFootnotes);
    
            var readFooter = targdoc.getElementById('readFooter');
        ...
        for (var i = 0; i < articleLinks.length; i+=1)
        {
            var articleLink  = articleLinks[i],
                ...
                refLink      = targdoc.createElement('a'),
                footnote     = targdoc.createElement('li'),
                linkDomain   = footnoteLink.host ? footnoteLink.host : targdoc.location.host,
    ...
    useRdbTypekit: function () {
        var rdbHead      = targdoc.getElementsByTagName('head')[0];
        var rdbTKScript  = targdoc.createElement('script');
        var rdbTKCode    = null;

        var rdbTKLink    = targdoc.createElement('a');
        ...
        if (readStyle === "style-athelas") {
            ...
            targdoc.getElementById("rdb-footer-right").appendChild(rdbTKLink);
        }
    ...
    **/
     ...
     *
     * @param page a targdoc to run upon. Needs to be a full targdoc, complete with body.
     * @return Element
    **/
    grabArticle: function (page) {
        ...
        for(var nodeIndex = 0; (node = allElements[nodeIndex]); nodeIndex+=1) {
            ...
            if (node.tagName === "DIV") {
                if (node.innerHTML.search(readability.regexps.divToPElements) === -1) {
                    var newNode = targdoc.createElement('p');
                    try {
        ...
        **/
        ...
        {
            topCandidate = targdoc.createElement("DIV");
        ...
        **/
        var articleContent        = targdoc.createElement("DIV");
        if (isPaging) {
            articleContent.id     = "readability-content";
        }
            ...
            **/
            ...
            if(append) {
                ...
                if(siblingNode.nodeName !== "DIV" && siblingNode.nodeName !== "P") {
                    ...
                    nodeToAppend = targdoc.createElement("DIV");
                    try {
    ...
    /**
     * Removes script tags from the targdoc.
     *
     * @param Element
    **/
    ...
    **/
    ...
    /**
     * Look for any paging links that may occur within the targdoc.
     * 
     * @param body
     * @return object (array)
    **/
    ...
    **/
    ...
    /**
     * Make an AJAX request for each page and append it to the targdoc.
    **/
    curPageNum: 1,
    ...
    appendNextPage: function (nextPageLink) {
        ...
        var articlePage       = targdoc.createElement("DIV");
        ...
        targdoc.getElementById("readability-content").appendChild(articlePage);
        ...
        (function(pageUrl, thisPage) {
            readability.ajax(pageUrl, {
                success: function(r) {
                    ...
                    var page = targdoc.createElement("DIV");
                     ...
                     *   Maybe in the future abstract out prepDocument to work for both the original targdoc and AJAX-added pages.
                    **/
                    ...
                    **/
                     ...
                     * Compare it against all of the the previous targdoc's we've gotten. If the previous
                     * targdoc contains exactly the innerHTML of this first paragraph, it's probably a duplicate.
                    **/
                    ...
                    if(firstP && firstP.innerHTML.length > 100) {
                        for(var i=1; i <= readability.curPageNum; i+=1) {
                            var rPage = targdoc.getElementById('readability-page-' + i);
    ...
    **/
    ...
    /**
     * Helper function to, in a cross compatible way, get or set the current scroll offset of the targdoc.
    ...
    **/
    scrollTop: function(scroll){
        ...
        else if(targdoc.targdocElement.clientHeight) {
            return targdoc.targdocElement.scrollTop;
        }
        else {
            return targdoc.body.scrollTop;
        }
    },
    
    /**
     * scrollTo - Smooth scroll to the point of scrollEnd in the targdoc.
     * @return void
    **/
    curScrollStep: 0,
     ...
     **/
    emailBox: function () {
        var emailContainerExists = targdoc.getElementById('email-container');
        if(null !== emailContainerExists)
        {
            return;
        }

        var emailContainer = targdoc.createElement("DIV");
        emailContainer.setAttribute('id', 'email-container');
        emailContainer.innerHTML = '<iframe src="'+readability.emailSrc + '?pageUrl='+encodeURIComponent(window.location)+'&pageTitle='+encodeURIComponent(targdoc.title)+'" scrolling="no" onload="readability.removeFrame()" style="width:500px; height: 490px; border: 0;"></iframe>';

        targdoc.body.appendChild(emailContainer);          
    },
    ...
    removeFrame: function () {
        ...
        {
            var emailContainer = targdoc.getElementById('email-container');
